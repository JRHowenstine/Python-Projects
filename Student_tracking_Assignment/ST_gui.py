...
# Import our other modules in order to have access to them
import ST_main
import ST_func
...
def load_gui(self):
    ...
    self.txt_fname.grid(row=1,column=0,rowspan=1,columnspan=2,padx=(30,40),pady=(0,0),sticky=N+E+W) # make the cell for text box double wide
    ...
    self.lstList1.bind('<<ListboxSelect>>',lambda event: ST_func.onSelect(self,event)) # bind is an event listener
    self.scrollbar1.config(command=self.lstList1.yview)
    self.scrollbar1.grid(row=1,column=5,rowspan=7,columnspan=1,padx=(0,0),pady=(0,0),sticky=N+E+S) # make the scrollbar and list box 7 rows high
    self.lstList1.grid(row=1,column=2,rowspan=7,columnspan=3,padx=(0,0),pady=(0,0),sticky=N+E+S+W) # make the list box 3 columns wide

    self.btn_add = tk.Button(self.master,width=12,height=2,text="Add",command=lambda: ST_func.addToList(self))
    self.btn_add.grid(row=10,column=0,padx=(25,0),pady=(45,10),sticky=W)
    self.btn_update = tk.Button(self.master,width=12,height=2,text="Update",command=lambda: ST_func.onUpdate(self))
    self.btn_update.grid(row=10,column=1,padx=(15,0),pady=(45,10),sticky=W)
    self.btn_delete = tk.Button(self.master,width=12,height=2,text="Delete",command=lambda: ST_func.onDelete(self))
    self.btn_delete.grid(row=10,column=2,padx=(15,0),pady=(45,10),sticky=W)
    self.btn_close = tk.Button(self.master,width=12,height=2,text="Close",command=lambda: ST_func.ask_quit(self))
