...
# import our modules to have access
import ST_main
import ST_gui
...
# catch if the user clicks on the window upper-right 'X' to ensure they want to close
def ask_quit(self):
    if messagebox.askokcancel("Exit program","Okay to exit application?"):
        # This closes the app
        self.master.destroy()
        os._exit(0) # makes sure when the program closes it frees up the memory it was using while operating
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1: # meaning our database is empty
...
def onClear(self):
    # clear the etxt in these textboxes
    self.txt_fname.delete(0,END)
    self.txt_lname.delete(0,END)
    self.txt_phone.delete(0,END)
    self.txt_email.delete(0,END)
...
def onUpdate(self):
    ...
    var_email = self.txt_email.get().strip()
    var_email = self.txt_course.get().strip()
